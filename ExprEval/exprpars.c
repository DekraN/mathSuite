...
/* Internal functions */
EXPRERRTYPE exprMultiParse(exprObj *obj, exprNode *node, exprToken *tokens, int count);
EXPRERRTYPE exprInternalParse(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end);
EXPRERRTYPE exprInternalParseAssign(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int index);
EXPRERRTYPE exprInternalParseAdd(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int index);
EXPRERRTYPE exprInternalParseSub(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int index);
EXPRERRTYPE exprInternalParseMul(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int index);
EXPRERRTYPE exprInternalParseDiv(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int index);
EXPRERRTYPE exprInternalParsePosNeg(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int index);
EXPRERRTYPE exprInternalParseExp(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int index);
EXPRERRTYPE exprInternalParseFunction(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int p1, int p2);
EXPRERRTYPE exprInternalParseVarVal(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end);
EXPRERRTYPE exprStringToTokenList(exprObj *obj, char *expr, exprToken **tokens, int *count);
...
void exprFreeTokenList(exprToken *tokens, int count)
    ...
    for(pos = 0; pos < count; ++pos)
        {
        if(tokens[pos].type == EXPR_TOKEN_VALUE)
        	mpfr_clear(tokens[pos].data.val);
                    ...
                    if(!comment)
                        ...
                        if(expr[pos] == '.' || isdigit(expr[pos]))
                            ...
                            if(pass == 0)
                                ...
                                mpfr_init_set_str(list[tpos].data.val, buf, 10, MPFR_RNDN);
...
EXPRERRTYPE exprMultiParse(exprObj *obj, exprNode *node, exprToken *tokens, int count)
    ...
    last = cur = 0; /* Not for last semicolon, but for first char of subexpr */
...
EXPRERRTYPE exprInternalParseAssign(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int index)
    ...
    */
    ...
    if(addr == NULL) /* Variable not in the list, add it */
        ...
        exprValListAdd(l, tokens[index - 1].data.str, tmp);
        mpfr_clear(tmp);
...
EXPRERRTYPE exprInternalParseFunction(exprObj *obj, exprNode *node, exprToken *tokens, int start, int end, int p1, int p2)
    ...
    err = exprFuncListGet(l, tokens[p1 - 1].data.str, &rt, &fptr, &rfptr, &mp_ctrl, &argmin, &argmax, &refargmin, &refargmax);
    if(err != EXPR_ERROR_NOERROR)
        {
    ...
    node->data.function.rfptr = rfptr;
    node->data.function.rt = rt;
    ...
    node->data.function.mp_ctrl = mp_ctrl;
                        ...
                        if(plevel == 0)
                            ...
                            if(tokens[lv].type == EXPR_TOKEN_AMPERSAND)
                                ...
                                if(addr == NULL)
                                    ...
                                    mpfr_t tmp;
                                    mpfr_init_set_ui(tmp, 0, MPFR_RNDN);
                                    exprValListAdd(vars, tokens[lv + 1].data.str, tmp);
                                    mpfr_clear(tmp);
    ...
    if(tokens[start].type == EXPR_TOKEN_IDENTIFIER)
        ...
        */
        ...
        if(addr == NULL) /* Variable not in the list, add it */
        	...
        	mpfr_t tmp;
            mpfr_init_set_ui(tmp, 0, MPFR_RNDN);
            exprValListAdd(l, tokens[start].data.str, tmp);
			mpfr_clear(tmp);
        ...
        mpfr_init_set(node->data.value.value, tokens[start].data.val, MPFR_RNDN);
        return EXPR_ERROR_NOERROR;
        }
    else
        {
