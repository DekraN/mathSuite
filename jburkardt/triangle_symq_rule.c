#ifndef __DISABLEDEEP_TRIANGLESYMQRULE

#include "../dutils.h"

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _quaenodes ( void * data)
/******************************************************************************/
/*
  Purpose:
  ...
  Parameters:
    ...
    Output, int NPTSOUTOUT, the number of points in the expanded set.
    Output, double XS2[NPTSOUTOUT], YS2[NPTSOUTOUT], WS2[NPTSOUTOUT],
    the coordinates and weights of the expanded set of points.
*/
{
	const dt3pitdt3pit * const s_data = data;
	const register dim_typ nptsout = s_data->a0;
	ityp * xsout = s_data->a1;
	ityp * ysout = s_data->a2;
	ityp * wsout = s_data->a3;
	const register dim_typ nptsoutout = s_data->a4;
	ityp * xs2 = s_data->a5;
	ityp * ys2 = s_data->a6;
	ityp * ws2 = s_data->a7;
	
	const ityp eps = 1.0E-12;
	dim_typ i;
	dim_typ ntot = 0;
	ityp w0;
	ityp x0;
	...
	for ( i = 0; i < nptsout; ++i )
	{
		...
		else if ( pow ( xsout[i], 2 ) < eps ||fabs ( ysout[i] - xsout[i] / sqrt ( 3.0 ) ) < sqrt ( eps ) )
		{
			x0 = xsout[i];
			y0 = ysout[i];
			w0 = wsout[i] / 3.00;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _quaenodes2 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _quaerotate ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	const _2it2pit * const s_data = data;
	const register ityp xin = s_data->a0;
	const register ityp yin = s_data->a1;
	ityp * xout = s_data->a2;
	ityp * yout = s_data->a3;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _quaecopy ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/*****************************************************************************/
__MATHSUITE __JBURKARDT  void *   _quaequad ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const register dim_typ itype = s_data->a0;
	const register dim_typ mmax = s_data->a1;
	const register dim_typ numnodes = s_data->a2;
	ityp * zs = s_data->a3;
	ityp * whts = s_data->a4;
	...
	*/
	nf = triangle_rule_full_size ( mmax );
	xf = ( ityp * ) malloc ( nf * sizeof ( ityp ) );
	yf = ( ityp * ) malloc ( nf * sizeof ( ityp ) );
	wf = ( ityp * ) malloc ( nf * sizeof ( ityp ) );
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_area ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	result = 0.50 * ( ( vert2[0] - vert1[0] ) * ( vert3[1] - vert1[1] ) - ( vert3[0] - vert1[0] ) * ( vert2[1] - vert1[1] ) );
	return &result;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _quaeinside ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const register ityp xsout = s_data->a0;
	const register ityp ysout = s_data->a1;
	const register dim_typ iitype = s_data->a2;
	...
	*/
	if ( iitype == 2 )
	{
		...
		hx
		*/
		if ( ysout < -1.00 / s - 1.0E-30 || xsout / s < ysout )
		nbool = false;
	}
	/*
	The 1/3 of triangle.
	*/
	else if ( iitype == 1 )
	{
		...
		if ( xsout <= 0.00 && -1.00 <= xsout &&  -1.00 / s <= ysout && ysout <= xsout / s )
			nbool = true;
		if ( iitype == 1 )
		{
			if ( 0.00 <= xsout && xsout <= 1.0 &&-1.00 / s <= ysout && ysout <= - xsout / s )
				nbool = true;
		}
	}
	/*
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _quaequad0 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const register dim_typ mmax = s_data->a0;
	const register dim_typ kk = s_data->a1;
	ityp * xnew = s_data->a2;
	ityp * ynew = s_data->a3;
	ityp * w = s_data->a4;
    ...
    */
    ...
    {
        case 1:
            triangle_rule01 ( xnew, ynew, w );
            break;
        case 2:
            triangle_rule02 ( xnew, ynew, w );
            break;
        case 3:
            triangle_rule03 ( xnew, ynew, w );
            break;
        case 4:
            triangle_rule04 ( xnew, ynew, w );
            break;
        case 5:
            triangle_rule05 ( xnew, ynew, w );
            break;
        case 6:
            triangle_rule06 ( xnew, ynew, w );
            break;
        case 7:
            triangle_rule07 ( xnew, ynew, w );
            break;
        case 8:
            triangle_rule08 ( xnew, ynew, w );
            break;
        case 9:
            triangle_rule09 ( xnew, ynew, w );
            break;
        case 10:
            triangle_rule10 ( xnew, ynew, w );
            break;
        case 11:
            triangle_rule11 ( xnew, ynew, w );
            break;
        case 12:
            triangle_rule12( xnew, ynew, w);
            break;
        case 13:
            triangle_rule13 ( xnew, ynew, w );
            break;
        case 14:
            triangle_rule14 ( xnew, ynew, w );
            break;
        case 15:
            triangle_rule15 ( xnew, ynew, w );
            break;
        case 16:
            triangle_rule16 ( xnew, ynew, w );
            break;
        case 17:
            triangle_rule17 ( xnew, ynew, w );
            break;
        case 18:
            triangle_rule18 ( xnew, ynew, w );
            break;
        case 19:
            triangle_rule19 ( xnew, ynew, w );
            break;
        case 20:
            triangle_rule20 ( xnew, ynew, w );
            break;
        case 21:
            triangle_rule21 ( xnew, ynew, w );
            break;
        case 22:
            triangle_rule22 ( xnew, ynew, w );
            break;
        case 23:
            triangle_rule23 ( xnew, ynew, w );
            break;
        case 24:
            triangle_rule24 ( xnew, ynew, w );
            break;
        case 25:
            triangle_rule25 ( xnew, ynew, w );
            break;
        case 26:
            triangle_rule26 ( xnew, ynew, w );
            break;
        case 27:
            triangle_rule27 ( xnew, ynew, w );
            break;
        case 28:
            triangle_rule28 ( xnew, ynew, w );
            break;
        case 29:
            triangle_rule29 ( xnew, ynew, w );
            break;
        case 30:
            triangle_rule30 ( xnew, ynew, w );
            break;
        case 31:
            triangle_rule31 ( xnew, ynew, w );
            break;
        case 32:
            triangle_rule32 ( xnew, ynew, w );
            break;
        case 33:
            triangle_rule33 ( xnew, ynew, w );
            break;
        case 34:
            triangle_rule34 ( xnew, ynew, w );
            break;
        case 35:
            triangle_rule35 ( xnew, ynew, w );
            break;
        case 36:
            triangle_rule36 ( xnew, ynew, w );
            break;
        case 37:
            triangle_rule37 ( xnew, ynew, w );
            break;
        case 38:
            triangle_rule38 ( xnew, ynew, w );
            break;
        case 39:
            triangle_rule39 ( xnew, ynew, w );
            break;
        case 40:
            triangle_rule40 ( xnew, ynew, w );
            break;
        case 41:
            triangle_rule41 ( xnew, ynew, w );
            break;
        case 42:
            triangle_rule42 ( xnew, ynew, w );
            break;
        case 43:
            triangle_rule43 ( xnew, ynew, w );
            break;
        case 44:
            triangle_rule44 ( xnew, ynew, w );
            break;
        case 45:
            triangle_rule45 ( xnew, ynew, w );
            break;
        case 46:
            triangle_rule46 ( xnew, ynew, w );
            break;
        case 47:
            triangle_rule47 ( xnew, ynew, w );
            break;
        case 48:
            triangle_rule48 ( xnew, ynew, w );
            break;
        case 49:
            triangle_rule49 ( xnew, ynew, w );
            break;
        case 50:
            triangle_rule50 ( xnew, ynew, w );
            break;
        default:
            return NULL;
    }
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule_compressed_size ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const register dim_typ mmax = *(dim_typ *) data;
	
    const dim_typ nnodes[50] =
    {
        1,  1,  2,  2,  3,
        3,  4,  5,  6,  7,
        8,  8, 10, 10, 12,
	...
	result = 1 <= mmax && mmax <= 50 ? nnodes[mmax-1] : USHRT_MAX;
    return &result;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule_full_size ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule01 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule02 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule03 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule04 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule05 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule06 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule07 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule08 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule09 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule10 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule11 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule12 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule13 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule14 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule15 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule16 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule17 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule18 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule19 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule20 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule21 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule22 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule23 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule24 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule25 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule26 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule27 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule28 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule29 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule30 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule31 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule32 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule33 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule34 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule35 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule36 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule37 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule38 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule39 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule40 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule41 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule42 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule43 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule44 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule45 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule46 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule47 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule48 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule49 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triangle_rule50 ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void   * _simplex_to_triangle ( void * data)
/******************************************************************************/
/*
  Purpose:
...
{
	ityp ** const a_data = data;
	ityp * tvert1 = a_data[0];
	ityp * tvert2 = a_data[1];
	ityp * tvert3 = a_data[2];
	ityp * s = a_data[3];
	
    dim_typ i;
    ityp *t;

    t = ( double * ) malloc (  sizeof ( ityp ) << 1 );

    #pragma omp parallel for num_threads(2)
    for ( i = 0; i < 2; ++i)
        t[i] = tvert1[i] * ( 1.00 - s[0] - s[1] )+ tvert2[i] * s[0]+ tvert3[i] * s[1];

    return t;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void   * _triangle_to_ref ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    rvert1[0] = - 1.00;
    rvert1[1] = - 1.00 / sqrt ( 3.00 );
    rvert2[0] = + 1.00;
    rvert2[1] = - 1.00 / sqrt ( 3.00 );
    rvert3[0] =   0.00;
    rvert3[1] =   2.00 / sqrt ( 3.00 );
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void   * _triangle_to_simplex ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
  ...
  ityp *s = ( ityp * ) malloc ( sizeof ( ityp ) << 1 );

  s[0] = ( ( tvert3[1] - tvert1[1] ) * ( t[0]      - tvert1[0] )- ( tvert3[0] - tvert1[0] ) * ( t[1]      - tvert1[1] ) )/ ( ( tvert3[1] - tvert1[1] ) * ( tvert2[0] - tvert1[0] )- ( tvert3[0] - tvert1[0] ) * ( tvert2[1] - tvert1[1] ) );
  s[1] = ( ( tvert2[0] - tvert1[0] ) * ( t[1]      - tvert1[1] )- ( tvert2[1] - tvert1[1] ) * ( t[0]      - tvert1[0] ) )/ ( ( tvert3[1] - tvert1[1] ) * ( tvert2[0] - tvert1[0] )- ( tvert3[0] - tvert1[0] ) * ( tvert2[1] - tvert1[1] ) );

  return s;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _trianmap ( void * data)
/******************************************************************************/
/*
  Purpose:
...
{
	const dt5pit * const s_data = data;
	const register dim_typ numnodes = s_data->a0;
	ityp * vert1 = s_data->a1;
	ityp * vert2 = s_data->a2;
	ityp * vert3 = s_data->a3;
	ityp * rnodes = s_data->a4;
	ityp * whts = s_data->a5;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _triasimp ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    const register ityp scale = 1.00 / sqrt ( 3.00 );

    *uout = 0.50 * ( x + 1.00 ) - 0.50 * scale * ( y + scale );
    *vout = 1.00 * scale * ( y + scale );

    return NULL;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _triasymq ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	const dt5pitdt * const s_data = data;
	const register dim_typ n = s_data->a0;
	ityp * vert1 = s_data->a1;
	ityp * vert2 = s_data->a2;
	ityp * vert3 = s_data->a3;
	ityp * rnodes = s_data->a4;
	ityp * weights = s_data->a5;
	const register dim_typ numnodes = s_data->a6;
	
    quaequad ( 0, n, rnodes, weights, numnodes );
