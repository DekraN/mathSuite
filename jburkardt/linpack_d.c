#ifndef __DISABLEDEEP_LINPACKD

#include "../dutils.h"

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _dcopy ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dasum ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _idamax ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dchdc ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	ityp * work = s_data->a3;
	dim_typ * ipvt = s_data->a4;
	dim_typ job = s_data->a5;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dchdd ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const pit2dt2pit2dt4pit * const s_data = data; 
	ityp * r = s_data->a0;
	const register dim_typ ldr = s_data->a1;
	const register dim_typ p = s_data->a2;
	ityp * x = s_data->a3;
	ityp * z = s_data->a4;
	const register dim_typ ldz = s_data->a5;
	const register dim_typ nz = s_data->a6;
	ityp * y = s_data->a7;
	ityp * rho = s_data->a8;
	ityp * c = s_data->a9;
	ityp * s = s_data->a10;
    ...
    */
    for ( j = 1; j <= nz; ++j )
    {
        ...
        if ( rho[j-1] < azeta )
        {
            ...
            rho[j-1] = rho[j-1] * sqrt ( 1.00 - pow ( azeta / rho[j-1], 2 ) );
    }
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _dchex ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	const pit4dtpit2dt2pitdt * const s_data = data;
	ityp * r = s_data->a0;
	dim_typ ldr = s_data->a1;
	const register dim_typ p = s_data->a2;
	dim_typ k = s_data->a3;
	dim_typ l = s_data->a4;
	ityp * z = s_data->a5;
	dim_typ ldz = s_data->a6;
	dim_typ nz = s_data->a7;
	ityp * c = s_data->a8;
	ityp * s = s_data->a9;
	dim_typ job = s_data->a10;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dchud ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dgbco ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const pit4dtpdtpit * const s_data = data;
	ityp * abd = s_data->a0;
	const register dim_typ lda = s_data->a1;
	const register dim_typ n = s_data->a2;
	const register dim_typ ml = s_data->a3;
	const register dim_typ mu = s_data->a4;
	dim_typ * ipvt = s_data->a5;
	ityp * z = s_data->a6; 
    ...
    */
    ...
    for ( j = 1; j <= n; ++j )
    {
        anorm = MAX ( anorm, dasum ( l, abd+is-1+(j-1)*lda, 1 ) );
        if ( ml + 1 < is )
            -- is;
        if ( j <= mu )
            ++ l;
    ...
    */
    ...
    for ( k = 1; k <= n; ++k )
    {
        ...
        if ( abs ( abd[m-1+(k-1)*lda] ) < abs ( ek - z[k-1] ) )
        {
            s = abs ( abd[m-1+(k-1)*lda] ) / abs ( ek - z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] = s * z[i-1];
        ek = s * ek;
        }
        ...
        if ( k+1 <= ju )
        {
            ...
            {
                -- mm ;
                sm += abs ( z[j-1] + wkm * abd[mm-1+(j-1)*lda] );
    ...
    */
    for ( k = n; 1 <= k; --k )
    {
        ...
        if ( k < m )
            z[k-1] += ddot ( lm, abd+m+(k-1)*lda, 1, z+k, 1 );
    ...
    */
    for ( k = n; 1 <= k; --k )
    {
        if ( abs ( abd[m-1+(k-1)*lda] ) < abs ( z[k-1] ) )
        {
            s = abs ( abd[m-1+(k-1)*lda] ) / abs ( z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
            ynorm *= s;
        }

        z[k-1] = abd[m-1+(k-1)*lda] ? z[k-1] / abd[m-1+(k-1)*lda] : 1.00;
    ...
    */
	...
	result = 0.00 + (anorm!=0)*(s * ynorm/anorm);
    return &result;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dgbdi ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	const pit4dtpipit * const s_data = data;
	ityp * abd = s_data->a0;
	const register dim_typ lda = s_data->a1;
	const register dim_typ n = s_data->a2;
	const register dim_typ ml = s_data->a3;
    ...
    for ( i = 1; i <= n; ++i )
    {
        if ( ipvt[i-1] != i )
            ...
            while ( abs ( det[0] ) < 1.00 )
            {
                det[0] *= 10.00;
                -- det[1];
            }

        while ( 10.00 <= abs ( det[0] ) )
        {
            det[0] /= 10.00;
            ++ det[1];
        }
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dgbfa ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const pit4dtpdt * const s_data = data;
	ityp * abd = s_data->a0;
	const register dim_typ lda = s_data->a1;
	const register dim_typ n = s_data->a2;
	const register dim_typ ml = s_data->a3;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dgbsl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	const pit4dtpdtpitdt * const s_data = data;
	ityp * abd = s_data->a0;
	const register dim_typ lda = s_data->a1;
	const register dim_typ n = s_data->a2;
	const register dim_typ ml = s_data->a3;
	...
	dim_typ * ipvt = s_data->a5;
	ityp * b = s_data->a6;
	const register dim_typ job = s_data->a7;
    ...
    else
    {
        ...
        */
        if ( 0 < ml )
        {
            ...
            {
                lm = MIN ( ml, n-k );
                b[k-1] += ddot ( lm, abd+m+(k-1)*lda, 1, b+k, 1 );
                l = ipvt[k-1];
                if ( l != k )
                {
                    t = b[l-1];
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dgeco ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    anorm = 0.00;
    for ( j = 1; j <= n; ++j )
        anorm = MAX ( anorm, dasum ( n, a+0+(j-1)*lda, 1 ) );
    /*
    ...
    */
    ...
    for ( k = 1; k <= n; ++k )
    {
        ...
        if ( abs ( a[k-1+(k-1)*lda] ) < abs ( ek - z[k-1] ) )
        {
            s = abs ( a[k-1+(k-1)*lda] ) / abs ( ek - z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
            ek *= s;
        }
        ...
        if ( k+1 <= n )
        {
            for ( j = k+1; j <= n; ++j)
            {
                sm += abs ( z[j-1] + wkm * a[k-1+(j-1)*lda] );
    ...
    */
    for ( k = n; 1 <= k; --k )
    {
        z[k-1] += ddot ( n - k, a+k+(k-1)*lda, 1, z+k, 1 );
    ...
    */
    for ( k = n; 1 <= k; --k )
    {
        if ( abs ( a[k-1+(k-1)*lda] ) < abs ( z[k-1] ) )
        {
            s = abs ( a[k-1+(k-1)*lda] ) / abs ( z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
            ynorm *= s;
        }
    ...
    */
	...
	result = 0.00 + (anorm!=0.00)*((s * ynorm)/anorm);
    return &result;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dgedi ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	const pit2dtpi2pitdt * const s_data = data; 
	ityp * a = s_data->a0;
	const register dim_typ lda = s_data->a1;
	const register dim_typ n = s_data->a2;
	int * ipvt = s_data->a3;
	ityp * det = s_data->a4;
	ityp * work = s_data->a5;
	const register dim_typ job = s_data->a6;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dgefa ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    ...
    ipvt[n-1] = n;
    result = a[n-1+(n-1)*lda] == 0.00 ? n : info;
    return &result;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dgesl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	ityp * b = s_data->a3;
	dim_typ * ipvt = s_data->a4;
	const register dim_typ job = s_data->a5;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _dgtsl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dpbco ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    for ( j = 1; j <= n; ++j )
    {
        ...
        {
            ++ k;
            z[k-1] = z[k-1] + abs ( abd[i-1+(j-1)*lda] );
        }
    }
    anorm = 0.00;
    for ( i = 1; i <= n; ++i)
        anorm = MAX ( anorm, z[i-1] );
    /*
    Factor.
    */
    ...
    */
    ...
    for ( k = 1; k <= n; ++k )
    {
        ...
        if ( abd[m+(k-1)*lda] < abs ( ek - z[k-1] ) )
        {
            s = abd[m+(k-1)*lda] / abs ( ek - z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
            ek *= s;
        }
        ...
        if ( k+1 <= j2 )
        {
            ...
            {
                -- i;
                sm = sm + abs ( z[j-1] + wkm * abd[i-1+(j-1)*lda] );
    ...
    */
    for ( k = n; 1 <= k; --k)
    {
        if ( abd[m+(k-1)*lda] < abs ( z[k-1] ) )
        {
            s = abd[m+(k-1)*lda] / abs ( z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
        }
        z[k-1] /= abd[m+(k-1)*lda];
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dpbdi ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    ...
    for ( i = 1; i <= n; ++i )
    {
        det[0] *=  abd[m+(i-1)*lda] * abd[m+(i-1)*lda];
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dpbfa ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    for ( j = 1; j <= n; ++j )
    {
        ...
        for ( k = mu; k <= m; ++k )
        {
            t = abd[k-1+(j-1)*lda]- ddot ( k-mu, abd+ik-1+(jk-1)*lda, 1, abd+mu-1+(j-1)*lda, 1 );
            t /= abd[m+(jk-1)*lda];
            abd[k-1+(j-1)*lda] = t;
            s = s + t * t;
            -- ik;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dpbsl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dpoco ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    ...
    for ( k = 1; k <= n; ++k)
    {
        ...
        if ( a[k-1+(k-1)*lda] < abs ( ek - z[k-1] ) )
        {
            s = a[k-1+(k-1)*lda] / abs ( ek - z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
            ek *= s;
        }
    ...
    */
    for ( k = n; 1 <= k; --k)
    {
        if ( a[k-1+(k-1)*lda] < abs ( z[k-1] ) )
        {
            s = a[k-1+(k-1)*lda] / abs ( z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
        }
        z[k-1] /= a[k-1+(k-1)*lda];
    ...
    */
    for ( k = 1; k <= n; ++k )
    {
        z[k-1] -= ddot ( k-1, a+0+(k-1)*lda, 1, z, 1 );
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dpodi ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const register dim_typ lda = s_data->a0;
	const register dim_typ n = s_data->a1;
	const register dim_typ job = s_data->a2;
	ityp * det = s_data->a3;
	ityp * a = s_data->a4;
    ...
    */
    if ( job / 10 != 0 )
    {
        ...
        for ( i = 1; i <= n; ++i )
        {
            det[0] *= a[i-1+(i-1)*lda] * a[i-1+(i-1)*lda];
    ...
    */
    ...
    for ( k = 1; k <= n; k++ )
        {
            a[k-1+(k-1)*lda] = 1.00 / a[k-1+(k-1)*lda];
            t = -a[k-1+(k-1)*lda];
            dscal ( k-1, t, a+0+(k-1)*lda, 1 );
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dpofa ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _dposl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dppco ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    ...
    for ( k = 1; k <= n; ++k)
    {
        ...
        if ( ap[kk-1] < abs ( ek - z[k-1] ) )
        {
            s = ap[kk-1] / abs ( ek - z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
            ek *= s;
        }
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dppdi ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dppfa ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dppsl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _dptsl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dqrdc ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	const pit3dtpitpipitdt * const s_data = data;
	ityp * a = s_data->a0;
	const register dim_typ lda = s_data->a1;
	const register dim_typ n = s_data->a2;
	const register dim_typ p = s_data->a3;
	ityp * qraux = s_data->a4;
	int * jpvt = s_data->a5;
	ityp * work = s_data->a6;
	const register dim_typ job = s_data->a7;
    ...
    for ( l = 1; l <= lup; l++ )
    {
        ...
        */
        ...
        if ( l != n )
        {
            ...
            if ( nrmxl != 0.00 )
            {
                if ( a[l-1+(l-1)*lda] != 0.00 )
                    nrmxl *= r8_sign ( a[l-1+(l-1)*lda] );

                dscal ( n-l+1, 1.00 / nrmxl, a+l-1+(l-1)*lda, 1 );
                a[l-1+(l-1)*lda] = 1.00 + a[l-1+(l-1)*lda];
                /*
                Apply the transformation to the remaining columns, updating the norms.
                */
                for ( j = l + 1; j <= p; ++j )
                {
                    t = -ddot ( n-l+1, a+l-1+(l-1)*lda, 1, a+l-1+(j-1)*lda, 1 )/ a[l-1+(l-1)*lda];
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dqrsl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const pit3dt7pitdt * const s_data = data; 
	ityp * a = s_data->a0;
	const register dim_typ lda = s_data->a1;
	const register dim_typ n = s_data->a2;
	const register dim_typ k = s_data->a3;
	ityp * qraux = s_data->a4;
	ityp * y = s_data->a5;
	ityp * qy = s_data->a6;
	ityp * qty = s_data->a7;
	...
	ityp * rsd = s_data->a9;
	ityp * ab = s_data->a10;
	const register dim_typ job = s_data->a11;
    ...
    if ( cr || cab )
    {
        for ( jj = 1; jj <= ju; ++jj )
        {
            ...
            if ( qraux[j-1] != 0.00 )
            {
                ...
                if ( cr )
                {
                    t = -ddot ( n-j+1, a+j-1+(j-1)*lda, 1, rsd+j-1, 1 )/ a[j-1+(j-1)*lda];
                    daxpy ( n-j+1, t, a+j-1+(j-1)*lda, 1, rsd+j-1, 1 );
                }

                if ( cab )
                {
                    t = -ddot ( n-j+1, a+j-1+(j-1)*lda, 1, ab+j-1, 1 )/ a[j-1+(j-1)*lda];
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dsico ( void * data)
/******************************************************************************/
/*
  Purpose:
    ...
    */
    ...
    while ( k != 0 )
    {
        ...
        if ( ks != 2 )
        {
            ...
            z[k-1] = a[k-1+(k-1)*lda] != 0.00 ? 1.00 : z[k-1] / a[k-1+(k-1)*lda];
        }
        else
        {
    ...
    */
    ...
    while ( k != 0 )
    {
        ...
        if ( ks != 2 )
        {
            ...
            z[k-1] = a[k-1+(k-1)*lda] != 0.0 ? z[k-1] / a[k-1+(k-1)*lda] : 1.00;
        }
        else
        {
            ak = a[k-1+(k-1)*lda] / a[k-2+(k-1)*lda];
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dsidi ( void * data)
/******************************************************************************/
/*
  Purpose:
            ...
            if ( 0 <= kpvt[k-1] )
            {
                ...
                */
                ...
                if ( 2 <= k )
                {
                    ...
                    a[k-1+(k-1)*lda] = a[k-1+(k-1)*lda]+ ddot ( k-1, work, 1, a+0+(k-1)*lda, 1 );
                }
                kstep = 1;
            }
            /*
            ...
            */
            else
            {
                ...
                if ( 2 <= k )
                {
                    ...
                    a[k-1+k*lda] = a[k-1+k*lda]+ ddot ( k-1, a+0+(k-1)*lda, 1, a+0+k*lda, 1 );
                    dcopy ( k-1, a+0+(k-1)*lda, 1, work, 1 );
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dsifa ( void * data)
/******************************************************************************/
/*
  Purpose:
    ...
    while ( 0 < k )
    {
        ...
        */
        else
        {
            ...
            imaxp1 = imax + 1;
            for ( j = imaxp1; j <= k; ++j )
                rowmax = MAX ( rowmax, abs ( a[imax-1+(j-1)*lda] ) );
            if ( imax != 1 )
            {
            ...
            if ( alpha * rowmax <= abs ( a[imax-1+(imax-1)*lda] ) )
                kstep = swap = 1;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _dsisl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _dspco ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    ...
    while ( k != 0 )
    {
        ...
        if ( ks != 2 )
        {
            if ( abs ( ap[kk-1] ) < abs ( z[k-1] ) )
            {
                s = abs ( ap[kk-1] ) / abs ( z[k-1] );
                for ( i = 1; i <= n; i++ )
                {
                    z[i-1] *= s;
                }
            ...
            z[k-1] = ap[kk-1] != 0.0 ? z[k-1] / ap[kk-1] : 1.00;
        }
        else
        {
            km1k = ik + k - 1;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dspdi ( void * data)
/******************************************************************************/
/*
  Purpose:
            ...
            if ( 0 <= kpvt[k-1] )
            {
                ...
                */
                ...
                if ( 2 <= k )
                {
                    ...
                    ap[kk-1] += ddot ( k-1, work, 1, ap+ik, 1 );
                }
                kstep = 1;
            }
            else
            {
                ...
                */
                ...
                if ( 1 <= km1 )
                {
                    ...
                    ap[kk-1] += ddot ( km1, work, 1, ap+ik, 1 );
                }
                kstep = 2;
            }
            /*
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dspfa ( void * data)
/******************************************************************************/
/*
  Purpose:
    ...
    for ( ; ; )
    {
        ...
        */
        else
        {
            ...
            for ( j = imaxp1; j <= k; ++j )
            {
                rowmax = MAX ( rowmax, abs ( ap[imj-1] ) );
                imj += j;
            }
            ...
            if ( imax != 1 )
            {
                ...
                jmim = jmax + im;
                rowmax = MAX ( rowmax, abs ( ap[jmim-1] ) );
            }

            imim = imax + im;

            if ( alpha * rowmax <= abs ( ap[imim-1] ) )
                kstep = swap = 1;
            else if ( alpha * colmax * ( colmax / rowmax ) <= absakk )
            {
                kstep = 1;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _dspsl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dsvdc ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const pit3dt3pitdtpitdtpitdt * const s_data = data;
	ityp * a = s_data->a0;
	const register dim_typ lda = s_data->a1;
	const register dim_typ m = s_data->a2;
	const register dim_typ n = s_data->a3;
	...
	ityp * e = s_data->a5;
	ityp * u = s_data->a6;
	const register dim_typ ldu = s_data->a7;
	ityp * v = s_data->a8;
	const register dim_typ ldv = s_data->a9;
	ityp * work = s_data->a10;
	const register dim_typ job = s_data->a11;
    ...
    for ( l = 1; l <= lu; ++l )
    {
        ...
        */
        if ( l <= nct )
        {
            ...
            {
                if ( a[l-1+(l-1)*lda] != 0.00 )
                    s[l-1] = r8_sign ( a[l-1+(l-1)*lda] ) * abs ( s[l-1] );
                dscal ( m-l+1, 1.00 / s[l-1], a+l-1+(l-1)*lda, 1 );
                a[l-1+(l-1)*lda] = 1.00 + a[l-1+(l-1)*lda];
            }
            s[l-1] *= -1;
        }
        ...
        for ( j = l+1; j <= n; ++j )
        {
            ...
            */
            if ( l <= nct && s[l-1] != 0.00 )
            {
                t = - ddot ( m-l+1, a+l-1+(l-1)*lda, 1, a+l-1+(j-1)*lda, 1 )/ a[l-1+(l-1)*lda];
        ...
        */
        ...
        if ( l <= nrt )
        {
            ...
            */
            ...
            {
                if ( e[l] != 0.00 )
                    e[l-1] = r8_sign ( e[l] ) * abs ( e[l-1] );
                dscal ( n-l, 1.00 / e[l-1], e+l, 1 );
                e[l] = 1.00 + e[l];
            }
    ...
    if ( wantu )
    {
        ...
        for ( ll = 1; ll <= nct; ll++ )
        {
            ...
            if ( s[l-1] != 0.0 )
            {
                for ( j = l+1; j <= ncu; j++ )
                {
                    t = - ddot ( m-l+1, u+(l-1)+(l-1)*ldu, 1, u+(l-1)+(j-1)*ldu, 1 )/ u[l-1+(l-1)*ldu];
                    daxpy ( m-l+1, t, u+(l-1)+(l-1)*ldu, 1, u+(l-1)+(j-1)*ldu, 1 );
                }

                dscal ( m-l+1, -1.00, u+(l-1)+(l-1)*ldu, 1 );
                u[l-1+(l-1)*ldu] = 1.00 + u[l-1+(l-1)*ldu];
                for ( i = 1; i <= l-1; i++ )
                    u[i-1+(l-1)*ldu] = 0.00;
            }
            else
    ...
    if ( wantv )
    {
        for ( ll = 1; ll <= n; ++ll )
        {
            ...
            if ( l <= nrt && e[l-1] != 0.00 )
                for ( j = l+1; j <= n; ++j )
                {
                    t = - ddot ( n-l, v+l+(l-1)*ldv, 1, v+l+(j-1)*ldv, 1 )/ v[l+(l-1)*ldv];
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dtrco ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	...
	const register dim_typ ldt = s_data->a0;
	const register dim_typ n = s_data->a1;
	const register dim_typ job = s_data->a2;
	ityp * z = s_data->a3;
	ityp * t = s_data->a4;
    ...
    */
    ...
    for ( j = 1; j <= n; ++j )
    {
        ...
        tnorm = MAX ( tnorm, dasum ( l, t+i1-1+(j-1)*ldt, 1 ) );
    }
    /*
    ...
    */
    ...
    for ( kk = 1; kk <= n; ++kk )
    {
        ...
        if ( abs ( t[k-1+(k-1)*ldt] ) < abs ( ek - z[k-1] ) )
        {
            s = abs ( t[k-1+(k-1)*ldt] ) / abs ( ek - z[k-1] );
            for ( i = 1; i <= n; ++i )
                z[i-1] *= s;
            ek *= s;
        }
        ...
        if ( kk != n )
        {
            ...
            for ( j = j1; j <= j2; ++j )
            {
                sm = sm + abs ( z[j-1] + wkm * t[k-1+(j-1)*ldt] );
    ...
    */
    for ( kk = 1; kk <= n; ++kk )
    {
        ...
        if ( abs ( t[k-1+(k-1)*ldt] ) < abs ( z[k-1] ) )
        {
            s = abs ( t[k-1+(k-1)*ldt] ) / abs ( z[k-1] );
            for ( i = 1; i <= n; ++i)
                z[i-1] *= s;
            ynorm *= s;
        }

        z[k-1] = t[k-1+(k-1)*ldt] ? z[k-1] / t[k-1+(k-1)*ldt] : 1.00;
        i1 = i + lower*k;
    ...
    */
	...
	result = 0.00 + (tnorm!=0.00)*((s * ynorm)/tnorm);
    return &result;
}

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dtrdi ( void * data)
/******************************************************************************/
/*
  Purpose:
    ...
    */
    if ( ( job % 10 ) != 0 )
    {
        ...
        for ( k = 1; k <= n; ++k )
        {
            ...
            t[k-1+(k-1)*ldt] = 1.00 / t[k-1+(k-1)*ldt];
            temp = -t[k-1+(k-1)*ldt];
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _dtrsl ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    else if ( kase == 3 )
    {
        ...
        {
            j = n - jj + 1;
            b[j-1] -= ddot ( jj-1, t+j+(j-1)*ldt, 1, b+j, 1 );
            b[j-1] /= t[j-1+(j-1)*ldt];
        }
    }
    /*
    ...
    */
    else if ( kase == 4 )
    {
        ...
        for ( j = 2; j <= n; ++j )
        {
            b[j-1] -= ddot ( j-1, t+0+(j-1)*ldt, 1, b, 1 );
            b[j-1] /= t[j-1+(j-1)*ldt];
        }
    }
