#ifndef __DISABLEDEEP_PADUA

#include "../dutils.h"

/******************************************************************************/
__MATHSUITE __JBURKARDT  void *  _padua_points_set ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void   * _padua_points ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    xy = ( ityp * ) malloc ( n * sizeof ( ityp ) << 1 );

    if ( l == 0 )
    {
        xy[0+0*2] = xy[1+0*2] = 0.00;
        return xy;
    }
    ...
    for ( i = 0; i <= l; ++i)
    {
        ...
        for ( j = 1; j <= j_hi; ++j )
        {
            ...
            if ( ( i % 2 ) == 0 )
            {
                ...
                {
                    angle2 = ( ityp ) ( (j<<1) - 1 ) * M_PI / ( ityp ) ( l + 1 );
                    xy[1+k*2] = cos ( angle2 );
                }
            }
            else
            {
                ...
                {
                    angle2 = ( ityp ) ( (j<<1) - 2 ) * M_PI / ( ityp ) ( l + 1 );
                    xy[1+k*2] = cos ( angle2 );
                }
            }
            ++ k;
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void   * _padua_weights_set ( void * data)
/******************************************************************************/
/*
  Purpose:
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void   * _padua_weights ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
    ...
    */
    te1 = ( ityp * ) malloc ( lp2h * lp2h * sizeof ( ityp ) );

    for ( j = 0; j < lp2h; ++j )
        for ( i = 0; i < lp2h; ++i )
        {
            angle = M_PI * ( ityp ) ( (i*j)<<2 ) / (ityp ) ( l );
            te1[i+j*lp2h] = cos ( angle );
        }

    for ( j = 0; j < lp2h; ++j )
        for ( i = 1; i < lp2h; ++i )
            te1[i+j*lp2h] *= sqrt ( 2.00 );

    to1 = ( double * ) malloc ( lp2h * lp1h * sizeof (ityp ) );
    ...
    for ( j = 0; j < lp1h; ++j )
        for ( i = 1; i < lp2h; ++i)
            to1[i+j*lp2h] *= sqrt ( 2.00 );


    te2 = ( ityp * ) malloc ( lp2h * lp3h * sizeof ( ityp ) );

    for ( j = 0; j < lp3h; ++j )
        for ( i = 0; i < lp2h; ++i )
        {
            angle = M_PI * ( ityp ) ((i*j)<<2 ) / ( ityp ) ( l + 1 );
            te2[i+j*lp2h] = cos ( angle );
        }

    for ( j = 0; j < lp3h; ++j )
        for ( i = 1; i < lp2h; ++i )
            te2[i+j*lp2h] *= sqrt ( 2.00 );

    to2 = ( ityp * ) malloc ( lp2h * lp2h * sizeof ( ityp ) );

    for ( j = 0; j < lp2h; ++j )
        for ( i = 0; i < lp2h; ++i)
        {
            angle = M_PI * ( ityp ) ( (i<<1) * ( (j<<1) + 1 ) )/ ( ityp ) ( l + 1 );
            to2[i+j*lp2h] = cos ( angle );
        }

    for ( j = 0; j < lp2h; ++j )
        for ( i = 1; i < lp2h; ++i )
            to2[i+j*lp2h] *= sqrt ( 2.00 );
    /*
    MOM: Moments matrix do even * even pairs.
    */
    mom = ( ityp * ) malloc ( lp2h * lp2h * sizeof ( ityp ) );

    for ( j = 0; j < lp2h; ++j )
    {
        mj = 2.00 * sqrt ( 2.00 ) / ( ityp ) ( 1 - pow ( j<<1, 2 ) );
        for ( i = 0; i < lp2h - j; ++i)
        {
            mi = 2.00 * sqrt ( 2.00 ) / ( ityp ) ( 1 - pow ( i<<1, 2 ) );
            mom[i+j*lp2h] = mi * mj;
        }
    }
    ...
    */
    tmtoe = ( ityp * ) malloc ( lp2h * lp2h * sizeof ( ityp ) );
    ...
    for ( j2 = 0; j2 < lp2h; ++j2 )
        for ( i2 = 0; i2 < lp2h - j2; ++i2 )
            for ( j = 0; j < lp2h; ++j )
                for ( i = 0; i < lp2h; ++i )
                tmtoe[i+j*lp2h] = tmtoe[i+j*lp2h]+ to2[i2+i*lp2h] * mom[j2+i2*lp2h] * te1[j2+j*lp2h];

    tmteo = ( ityp * ) malloc ( lp3h * lp1h * sizeof ( ityp ) );
    ...
    for ( j2 = 0; j2 < lp2h; ++j2)
        for ( i2 = 0; i2 < lp2h - j2; ++i2 )
            for ( j = 0; j < lp1h; ++j )
                for ( i = 0; i < lp3h; ++i )
                    tmteo[i+j*lp3h] = tmteo[i+j*lp3h]+ te2[i2+i*lp2h] * mom[j2+i2*lp2h] * to1[j2+j*lp2h];
    /*
    W1 and W2: Interpolation weight matrices.
    */
    w1 = ( ityp * ) malloc ( lp2h * lp2h * sizeof ( ityp ) );

    for ( j = 0; j < lp2h; ++j )
        for ( i = 0; i < lp2h; ++i )
            w1[i+j*lp2h] = 2.00 / ( ityp ) ( l * ( l + 1 ) );
    ...
    w2 = ( ityp * ) malloc ( lp3h * lp1h * sizeof ( ityp ) );

    for ( j = 0; j < lp1h; ++j )
        for ( i = 0; i < lp3h; ++i )
            w2[i+j*lp3h] = 2.00 / ( ityp ) ( l * ( l + 1 ) );
    ...
    */
    for ( j = 0; j < lp2h; ++j )
        for ( i = 0; i < lp2h; ++i )
            w1[i+j*lp2h] *= tmtoe[i+j*lp2h];

    for ( j = 0; j < lp1h; ++j )
        for ( i = 0; i < lp3h; ++i )
            w2[i+j*lp3h] *= tmteo[i+j*lp3h];
    /*
