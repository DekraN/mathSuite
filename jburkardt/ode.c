...
(
  ...
  ityp p[static neqn],
  ityp yp[static neqn],
  ityp ypout[static neqn],
  ityp phi[static neqn<<4],
  ityp alpha[static 12],
  ityp beta[static 12],
  ityp sig[static 13],
  ityp v[static 12],
  ityp w[static 12],
...
*/
{
    ...
    */
    fouru = 4.00 * r8_epsilon ( );

    if ( neqn < 1 || *t == tout || relerr < 0.00 || abserr < 0.00 || eps <= 0.00 || *iflag == 0 )
    {
        *iflag = 6;
        return;
    }
    ...
    *iflag = abs ( *iflag );
    ...
    if ( *iflag != 1 && (*t != *told || *iflag < 2 || 5 < *iflag ))
    {
        *iflag = 6;
        return;
    }
    ...
    */
    if ( *iflag == 1 || *isnold < 0 || *delsgn * del <= 0.00 )
    {
        *start = 1;
        *x = *t;
        for ( l = 1; l <= neqn; ++l )
            yy[l-1] = y[l-1];
        *delsgn = r8_sign ( del );
        *h = MAX ( abs ( tout - *x ), fouru * abs ( *x ) ) * r8_sign ( tout - *x );
    }
    /*
    ...
    */
    for ( ; ; )
    {
        if ( absdel <= abs ( *x - *t ) )
        {
            ode_intrp ( *x, yy, tout, y, ypout, neqn, *kold, phi, psi );
            *iflag = 2;
            *t = tout;
            *told = *t;
            *isnold = isn;
        ...
        */
        if ( isn <= 0 && abs ( tout - *x ) < fouru * abs ( *x ) )
        {
            *h = tout - *x;
            f ( *x, yy, yp );
            for ( l = 1; l <= neqn; ++l)
        ...
        */
        *h = MIN ( abs ( *h ), abs ( tend - *x ) ) * r8_sign ( *h );

        for ( l = 1; l <= neqn; ++l)
            wt[l-1] = releps * abs ( yy[l-1] ) + abseps;

        ode_step ( x, yy, f, neqn, h, &eps, wt, start,
        hold, (dim_typ*)k, kold, &crash, phi, p, yp, psi,
...
/******************************************************************************/
__MATHSUITE __JBURKARDT  void *   _ode_intrp ( void * data)
/******************************************************************************/
/*
  Purpose:
...
*/
{
	const itpitit2pit2dt2pit * const s_data = data;
	ityp x = s_data->a0; 
	ityp * y = s_data->a1;
	ityp xout = s_data->a2;
	ityp * yout = s_data->a3;
	ityp * ypout = s_data->a4;
	const register dim_typ neqn = s_data->a5;
	const register dim_typ kold = s_data->a6;
	ityp * phi = s_data->a7;
	ityp * psi = s_data->a8;
    ...
    */
    ...
    for ( j = 1; j <= ki; ++j )
    {
        ...
        for ( k = 0; k < neqn; ++k )
        {
            yout[k] += g[i-1] * phi[k+(i-1)*neqn];
            ypout[k] += rho[i-1] * phi[k+(i-1)*neqn];
        }
    }
...
*/
{
    const dim_typ ialpha = 1;
    const dim_typ ibeta = 13;
    const dim_typ idelsn = 93;
    const dim_typ ig = 62;
    const dim_typ ih = 89;
    const dim_typ ihold = 90;
    int ip;
    const dim_typ iphase = 75;
    int iphi;
    const dim_typ ipsi = 76;
    const dim_typ isig = 25;
    const dim_typ istart = 91;
    const dim_typ itold = 92;
    const dim_typ iv = 38;
    const dim_typ iw = 50;
    int iwt;
    const dim_typ ix = 88;
    ...
    if ( abs ( *iflag ) != 1 )
    {
        start = ( 0.00 < work[istart-1] );
        phase1 = ( 0.00 < work[iphase-1] );
        nornd = ( iwork[1] != -1 );
    }
 ...
 (
    ...
    work[istart-1] = -1.00 + ((start!=0)<<1);
    work[iphase-1] = -1.00 + ((phase1!=0)<<1);
    iwork[1] = -1 + ((nornd!=0)<<1);
    return;
}
...
*/
{
    ...
    ityp two[13] =
    {
        ...
        64.00,  128.00,  256.00, 512.00, 1024.00,
        2048.00, 4096.00, 8192.00
    };
    ityp twou;
    ityp xold;

    twou = 2.00 * r8_epsilon ( );
    fouru = 2.00 * twou;
    /*
    ...
    */
    if ( *start )
    {
        ...
        total = 0.00;
        for ( l = 1; l <= neqn; ++l )
            total += pow ( yp[l-1] / wt[l-1], 2 );
        total = sqrt ( total );
        absh = abs ( *h );
        if ( *eps < 16.0 * total * *h * *h )
            absh = 0.25 * sqrt ( *eps / total );
        *h = MAX ( absh, fouru * abs ( *x ) ) * r8_sign ( *h );
        *hold = 0.00;
        *k = *phase1 = *nornd = 1;
        *kold = *start = 0;

        if ( p5eps <= 100.00 * round )
        {
            *nornd = 0;
            for ( l = 1; l <= neqn; ++l )
                phi[l-1+14*neqn] = 0.00;
        }
    }
    ifail = 0;
    /*
    ...
    */
    for ( ; ; )
    {
        ...
        */
        if ( *ns <= *k )
        {
            beta[*ns-1] = sig[nsp1-1] = 1.00;
            alpha[*ns-1] = 1.00 / ( ityp ) ( *ns );
            temp1 = *h * ( ityp ) ( *ns );
            ...
            for ( i = nsp1; i <= *k; ++i )
            {
                ...
                temp1 = temp2 + *h;
                alpha[i-1] = *h / temp1;
                sig[i] = ( ityp ) ( i ) * alpha[i-1] * sig[i-1];
            }
            psi[*k-1] = temp1;
            /*
        ...
        */
        erkm2 = erkm1 = erk = 0.00;

        for ( l = 1; l <= neqn; ++l )
        {
            if ( 0 < km2 )
                erkm2 += pow ( ( phi[l-1+(km1-1)*neqn] + yp[l-1] - phi[l-1+0*neqn] ) / wt[l-1], 2 );

            if ( 0 <= km2 )
                erkm1 += pow ( ( phi[l-1+(*k-1)*neqn] + yp[l-1] - phi[l-1+0*neqn] ) / wt[l-1], 2 );
            erk += pow ( ( yp[l-1] - phi[l-1+0*neqn] ) / wt[l-1], 2 );
        }
        ...
        */
        if ( 0 < km2 && MAX ( erkm1, erkm2 ) <= erk )
            knew = km1;
        else if ( 0 == km2 && erkm1 <= 0.50 * erk )
            knew = km1;
        /*
        ...
        *x = xold;
        for ( i = 1; i <= *k; ++i )
            for ( l = 1; l <= neqn; ++l )
                phi[l-1+(i-1)*neqn] -= phi[l-1+i*neqn] / beta[i-1];
    ...
    *phase1 = 0;
    ...
    {
        for ( l = 1; l <= neqn; ++l )
            erkp1 += pow ( phi[l-1+(kp2-1)*neqn] / wt[l-1], 2 );
    ...
    */
    ...
    if ( !(*phase1) )
    {
        if ( p5eps < erk * two[*k] )
        {
            ...
            {
                temp2 = ( ityp ) ( *k + 1 );
                r = pow ( p5eps / erk, 1.00 / temp2 );
                hnew = absh * MAX ( 0.50, MIN ( 0.90, r ) );
                hnew = abs ( MAX ( hnew, fouru * abs ( *x ) ) ) * r8_sign ( *h );
            }
        }
    }
    *h = hnew;
