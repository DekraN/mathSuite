// MathSuite Mathematical LIBRARY v1.00
// 06/03/2016 by Wesker aka DekraN
...
{
    {
		...
		IDENTIFIER_R8SINDEG,   
		IDENTIFIER_R8SPENCE,   
		IDENTIFIER_FEETTOMETERS,   
		IDENTIFIER_POUNDSTOKILOGRAMS,  
		IDENTIFIER_BESSJ0, 
		IDENTIFIER_BESSJ1,  
		IDENTIFIER_BESSY0,
		IDENTIFIER_BESSY1,
		...
		IDENTIFIER_BESSK0,
		IDENTIFIER_BESSK1,
		IDENTIFIER_R8NORMAL01CDFINVERSE,  
		IDENTIFIER_DIGAMA,
		IDENTIFIER_PPND, 
		IDENTIFIER_TRIGAMMA,  
		IDENTIFIER_ALNGAM, 
		...
		IDENTIFIER_R8BESK1E,
		IDENTIFIER_R8LGMC,     
		IDENTIFIER_CIRCLEAREA2D,
		IDENTIFIER_COTRAD,    
		IDENTIFIER_HEXAGONAREA2D,
		IDENTIFIER_SPHEREIMPAREA3D,   
		IDENTIFIER_SPHEREIMPVOLUME3D,   
		IDENTIFIER_R8GAMMALOG,
		IDENTIFIER_R8EXPONENTIAL01PDF,
		IDENTIFIER_AGUD,
		IDENTIFIER_GUD,  
		IDENTIFIER_R8PSI,  
		IDENTIFIER_ZETA, 
		IDENTIFIER_BESSELI0,  
		IDENTIFIER_NORMAL01CDF, 
		IDENTIFIER_NORMAL01PDF,  
		IDENTIFIER_R8FRACTIONAL,   
		IDENTIFIER_R8REVERSEBYTES,        
		IDENTIFIER_R8GAMMA01SAMPLE,
		IDENTIFIER_R8CHISAMPLE
    },
    {
        sin,
        cos,
		...
		hexagon_area_2d,
		sphere_imp_area_3d,   
		sphere_imp_volume_3d,   
		r8_gamma_log,
		r8_exponential_01_pdf,
		agud, 
		gud,   
		r8_psi,  
		zeta, 
...
__MSNATIVE_ char * const __system __export binNumComp(ityp a)
{
    ...
    printf("Decimal ONE'S COMPLEMENT: %d;\nBinary ONE'S COMPLEMENT is: %s.\n\n", ~num, c);
...
__MSNATIVE_ bool __export _secondGradeEquationSolver(ityp *restrict abc, ityp root[static 2])
{
...
__MSUTIL_ inline bool __system __export isPrimeHISP(register uint64_t n)
{
    ...
    do
    {
        if(!(result = n%i)) return false; // break;
        ++ i;
    }
    while(n != i);
    if(n == i) return true;
...
__MSNATIVE_ const inline ityp __system __export answer_to_life_the_universe_and_everything(void)
{
	return (ityp) ANSWER_TO_LIFE_THE_UNIVERSE_AND_EVERYTHING;
}
...
__MSNATIVE_ inline ityp __export math_mean(uint64_t dim, ityp vector[static dim])
{
...
{
	const register ityp media = math_mean(dim, vector);
...
__MSNATIVE_ inline ityp __export math_variance2(uint64_t dim, ityp vector[static dim])
{
	const register ityp media = math_mean(dim, vector);
	ityp register res = 0.00;
...
{
	const register ityp media[2] =
	{
		math_mean(dim, vector1),
		math_mean(dim, vector2)
	};
...
__MSNATIVE_ inline ityp __export math_covariance2(uint64_t dim, ityp vector1[static dim], ityp vector2[static dim])
{
	const register ityp media[2] =
	{
		math_mean(dim, vector1),
...
__MSNATIVE_ inline ityp __export math_stdcod(uint64_t dim, ityp vector1[static dim], ityp vector2[static dim])
{
	const register ityp media[2] =
	{
		math_mean(dim, vector1),
...
__MSNATIVE_ inline ityp __export math_stdcod2(uint64_t dim, ityp vector1[static dim], ityp vector2[static dim])
{
	const register ityp media[2] =
	{
		math_mean(dim, vector1),
...
__MSNATIVE_ inline ityp __export math_stddev2(uint64_t dim, ityp vector[static dim])
{
	const register ityp media = math_mean(dim, vector);
	ityp register res = 0.00;
...
__MSNATIVE_ inline ityp __export math_pearson(uint64_t dim, ityp vector1[static dim], ityp vector2[static dim])
{
	const register ityp media[2] =
	{
		math_mean(dim, vector1),
	...
	return (res/dim)*(math_stddev(dim,vector1)*math_stddev(dim,vector2));
}

__MSNATIVE_ inline ityp __export math_pearson2(uint64_t dim, ityp vector1[static dim], ityp vector2[static dim])
{
	const register ityp media[2] =
	{
		math_mean(dim, vector1),
	...
	return (res/(dim-1))*(math_stddev(dim,vector1)*math_stddev(dim,vector2));
}
...
__MSNATIVE_ bool __export math_outlier2(uint64_t dim, uint64_t outlier_idx, ityp outlier_constant, ityp vector[static dim])
{
...
__MSNATIVE_ inline ityp __export math_geomean(uint64_t dim, ityp vector[static dim])
{
    return (pow(productory(dim, PRODUCTORY_MUL, vector), (1/dim)));
}

__MSNATIVE_ inline ityp __export math_armean(uint64_t dim, ityp vector[static dim])
{
    #pragma omp parallel for
    for(uint64_t i=0; i<dim; ++i)
        vector[i] = (1/vector[i]);
...
__MSNATIVE_ inline ityp __export math_powmean(uint64_t dim, uint64_t power, ityp vector[static dim])
{
	#pragma omp parallel for
    for(uint64_t i=0; i<dim; ++i)
        vector[i] = mpow2(vector[i], power);

    return (pow((1/dim)*summation(dim, SUMMATION_SUM, vector), (1/power)));
}

__MSNATIVE_ inline ityp __export math_scale(uint64_t dim, ityp vector[static dim])
{
    return ((_MIN(dim, vector)+_MAX(dim, vector))*0.50);
}
...
__MSNATIVE_ inline ityp __export math_median(uint64_t dim, ityp vector[static dim])
{
    qsort(vector, dim, sizeof(ityp), cmpfunc);
...
{
    return grad + 273.15*(1.00-(mode<<1));
}
...
{
    return (value * (mpow2(3.60, 1.00-(mode<<1))));
}
...
__MSNATIVE_ inline bool __system __export isEqualMatrix(ityp *matrix1, ityp *matrix2, const register dim_typ dim[static 2])
{
    dim_typ i, j;
...
__MSNATIVE_ inline ityp __system __export harris(const register ityp alpha, const register dim_typ dim, ityp *restrict a)
{
	return det(a, dim, NULL) - alpha*pow(_matrixTrace(a,dim),2);
}

__MSNATIVE_ inline ityp __system __export harris2(const register ityp alpha, ityp *restrict a)
{
	return det(a, 2, NULL) - alpha*pow(_matrixTrace(a,2),2);
}

__MSNATIVE_ inline bool __system __export t_harris(const register ityp alpha, const register ityp thresh, const register dim_typ dim, ityp *restrict a)
{
	return det(a, dim, NULL) - alpha*pow(_matrixTrace(a,dim),2) >= thresh;
}

__MSNATIVE_ inline bool __system __export t_harris2(const register ityp alpha, const register ityp thresh, ityp *restrict a)
{
	return det(a, 2, NULL) - alpha*pow(_matrixTrace(a,2),2) >= thresh;
}
...
__MSNATIVE_ short _MS__private __system __export _routhTable(ityp **table, const register dim_typ dim, fsel_typ *nullrow)
{
	...
	for(i=2; i<dim; ++i)
	{
		...
		*(tmp + 2) = *((*table)  + columns*(i-1));
		for(j=0; j<columns-1; ++j)
		{
			*(tmp + 1) = *((*table) + columns*(i-2) + j+1);
			*(tmp + 2 +1) = *((*table) + columns*(i-1) + j+1);
			*((*table) + columns*i + j) = pivot * det(tmp, 2, NULL);
		}
	}
...
__MSNATIVE_ sel_typ _MS__private __system __export _juryTable(ityp **table, const register dim_typ dim)
{
	...
	for(k=1,i=3; i<rows; i+=2,++k)
	{
		...
		*(tmp + 2) = *((*table) + dim*(i-2));
		...
		for(j=0; j<dim-k; ++j)
		{
			...
			*(tmp + 2 +1) = *((*table) + dim*(i-2) + j+1);
			*((*table) + (dim*i) + j) = det(tmp, 2, NULL);
			// if(i != rows-1)
	...
	*(tmp + 2) = *((*table) + dim*(rows-2));
	#pragma omp parallel for
	for(i=0; i<3; ++i)
	{
		*(tmp + 1) = *((*table) + dim*(rows-3) + i+1);
		*(tmp + 2 +1) = *((*table) + dim*(rows-2) + i+1);
		*((*table) + dim*(rows-1) + 2-i) = det(tmp, 2, NULL);
	}

	if(fabs(*((*table) + dim*(rows-1))) > fabs(*((*table) + dim*(rows-1) + 2)))
		++ jtest;
...
__MSNATIVE_ __MSUTIL_ sel_typ _MS__private __system __export _matrixEigenValues(ityp *restrict m, ityp *restrict l, ityp *restrict vc, const register dim_typ n)
{
	...
	while(true)
	{
		...
		ityp th = 0.5*atan(2* *(m2 + n*i + j)/(*(m2 + n*i + i) - *(m2 + n*j + j)));
		{
...
__MSNATIVE_ inline void _MS__private __system __export _matrixSub(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
...
__MSNATIVE_ void _MS__private __system __export _matrixCSub(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ void _MS__private __system __export _matrixQSub(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ void _MS__private __system __export _matrixOSub(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ void _MS__private __system __export _matrixSSub(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ inline void _MS__private __system __export _matrixAdd(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ void _MS__private __system __export _matrixCAdd(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ void _MS__private __system __export _matrixQAdd(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ void _MS__private __system __export _matrixOAdd(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ void _MS__private __system __export _matrixSAdd(ityp **matrix1, ityp **matrix2, ityp **matrix_sum, const register dim_typ dim[static 2])
{
	dim_typ i, j;
	register dim_typ idx;
...
__MSNATIVE_ inline void __system __call_OSMM(ityp **matrix1, ityp **matrix2, ityp **matrix_product, const register dim_typ dim [static MAX_MATRICES], const register sel_typ algunits,
	...
	void (* const sumFunc)(ityp **, ityp **, ityp **, const register dim_typ [static 2]),
	void (* const subFunc)(ityp **, ityp **, ityp **, const register dim_typ [static 2]))
{
...
__MSNATIVE_ __MSUTIL_ void __system __export mmult_fast(const register dim_typ N, const register sel_typ algunits, ityp **X, ityp **Y,  ityp **Z,
	...
	void (* const matrixAddFunc)(ityp **, ityp **, ityp **, const register dim_typ [static 2]),
	void (* const matrixSubFunc)(ityp **, ityp **, ityp **, const register dim_typ [static 2]))
{
...
__MSNATIVE_ void _MS__private __system __export _matrixSMultiplication(ityp **matrix1, ityp **matrix2, ityp **matrix_product, const register dim_typ dim[static 3])
{
    dim_typ i, j, k;
	register dim_typ idx[MAX_MATRICES];
...
__MSNATIVE_ inline void _MS__private __system __export _matrixKProduct(ityp **matrix1, ityp **matrix2, ityp **matrix_product, register dim_typ dim[static 2][2])
{
    dim_typ i, j, k, l;
...
__MSNATIVE_ void _MS__private __system __export _matrixKCProduct(ityp **matrix1, ityp **matrix2, ityp **matrix_product, register dim_typ dim[static 2][2])
{
    dim_typ i, j, k, l;
    register dim_typ idx[MAX_MATRICES];
    const register dim_typ pdim = dim[FIRST_MATRIX][COLUMNS]*dim[SECOND_MATRIX][COLUMNS];
...
__MSNATIVE_ void _MS__private __system __export _matrixKQProduct(ityp **matrix1, ityp **matrix2, ityp **matrix_product, register dim_typ dim[static 2][2])
{
    dim_typ i, j, k, l;
    register dim_typ idx[MAX_MATRICES];
    const register dim_typ pdim = dim[FIRST_MATRIX][COLUMNS]*dim[SECOND_MATRIX][COLUMNS];
...
__MSNATIVE_ void _MS__private __system __export _matrixKOProduct(ityp **matrix1, ityp **matrix2, ityp **matrix_product, register dim_typ dim[static 2][2])
{
    dim_typ i, j, k, l;
    register dim_typ idx[MAX_MATRICES];
    const register dim_typ pdim = dim[FIRST_MATRIX][COLUMNS]*dim[SECOND_MATRIX][COLUMNS];
...
__MSNATIVE_ void _MS__private __system __export _matrixKSProduct(ityp **matrix1, ityp **matrix2, ityp **matrix_product, register dim_typ dim[static 2][2])
{
