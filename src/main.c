...
///                              mathSuite v5.55 --- by Marco Chiarelli aka DekraN aka Wesker013                  ///
///        LAST UPDATE: 09:20 of 15/05/2013 by Myself. CHANGELOG v2.5: Fixed every imaginable bug and started     ///
...
///Optimized Code and Fixed some bugs into matrixProduct, precisely in the ALGEBRA_MATRIXPOWER subProgram. Now the///
/// product system is more efficient and readable. Now ALGEBRA_MATRIXPOWER gives in OUTPUT correct results.       ///
...
///ALGEBRA_MATRIXPOWER MACRO subProgram. Now it's possible to execute matrixProduct() function and all of its Macro///
...
/// memory, such as ENVS, MATRICES, LOGS and LAYOUTS. Added a new system (WINDOWS Only) of Colors Management.     ///
...
/// that allows you to load different Lists Items (ENVS, MATRICES, LOGS and LAYOUTS), simply by putting its fnames///
/// in a .msinf file. Program will load the Items Instances after recognizing them by its extensions. Default ones///
/// I remember, are .msvl for VARLISTS, .msmat for MATRICES, .LOG for Logs, .INI for LAYOUTS. An example-default  ///
...
///    . Added MAIN_ADVANCEDCALCULATOR subProgram, which presents these Children Subprograms: SecondGrade Equations///
...
/// ityp (Standard MS double Type). Added a basilar Printing System for ENVS, MATRICES, LOGS, SysLOG and LAYOUTS, ///
...
int main(int argc, char **argv)
{
        ...
        access(exit_char) = INITIALIZING_EXIT_CHAR;
        ...
        access(mhss) = access(sigresult) = false;

    #endif

    #if WINOS
    {
        _colINILoader(access(colors_path));
        ...
        /// SetColor(20); // testing color handle
    }
    #endif
    ...
    {
        printErr(2, "FATAL ERROR: Unable to force "DEFAULT_PATHLIST_FILE_EXTENSION" File opening: %s", path);
        system("PAUSE");
        return MSINF_FAULT_ERROR;
    }

    sel_typ mode;

    if(extension && (mode = checkItemTypeByExtension(extension)) != MAX_LISTS)
    ...
    progInfo(WITHOUT_DESCRIPTION); // Visualizza le Informazioni del Programma

    updInfo();

    // Setting (Sub)Programs Description
